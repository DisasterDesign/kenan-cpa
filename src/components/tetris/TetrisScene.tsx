...
const ROWS = 10;
const BASE_ROTATION_Y = 0.25;
const BASE_TILT_X = 0.1;
const BASE_TILT_Z = -0.05;
...
export default function TetrisScene({ gameState }: TetrisSceneProps) {
  ...
  useFrame(() => {
    ...
    groupRef.current.rotation.y = THREE.MathUtils.lerp(
      ...
      BASE_ROTATION_Y + mouseRef.current.x * 0.08,
      0.04
    );
    groupRef.current.rotation.x = THREE.MathUtils.lerp(
      groupRef.current.rotation.x,
      BASE_TILT_X + (-mouseRef.current.y * 0.05),
      0.04
    );
  });
  ...
  return (
    <>
      ...
      <group position={[0, 0, 0]} scale={isMobile ? 0.65 : 1}>
